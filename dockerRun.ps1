...
docker run --rm -it -p 8080:8080 -e DATABASE_URL="jdbc:postgresql://host.docker.internal:5432/befriend-local" -e DATABASE_USERNAME=$env:DATABASE_USERNAME -e DATABASE_PASSWORD=$env:DATABASE_PASSWORD befrined/befriend